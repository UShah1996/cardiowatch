"""
random_forest.py — Random Forest baseline with 5-fold CV + 95% CI reporting.
Saves trained model and scaler to data/processed/ for dashboard use.
"""
...
def train_and_evaluate():
    ...
    print("Random Forest — 5-fold CV (mean ± std):")
    ...
    # ── 95% CI on CV results (t-distribution, k-1 df) ─────────────────
    ...
    print(f"RF model saved  → {PROCESSED_DIR}/rf_model.pkl")
    ...
    print(f"\nRandom Forest — Test set (n={len(y_te)}, threshold=0.5):")
    ...
    # Bootstrap CI on the test set — quantifies uncertainty from small n=92
    bootstrap_all_metrics(
        np.array(y_te), y_proba,
        threshold=0.5,
        label=f'Random Forest Test Set (n={len(y_te)})',
        n_boot=2000,
    )
