...
%
% >>> NUMBERS: every value below traces to docs/results/<run_id>/. Cells marked
%     "\XX" are produced by the pre-registered cross-device run (job 14/15 ->
%     crossdevice_stats.json) and MUST be filled from that file before
%     submission; they are intentionally NOT guessed here.
...
\title{Device-Agnostic Screening for Atrial Fibrillation on Consumer
Wearables: A Pre-Registered, Leakage-Safe Cross-Device Evaluation for Stroke
Prevention}
...
\subsection{Why device-agnostic features transfer (mechanism)}
A device-separability analysis (can a classifier identify the recording device
from each representation?) explains the gap: the CNN-LSTM embedding is strongly
device-separable, whereas the RR/HRV feature vector is close to device-invariant.
The learned waveform representation encodes the hardware; the timing
representation does not. Full per-device-pair numbers are in
\texttt{representation\_shift.json}.
